  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <Link href="/buyer/orders" className="text-sm text-gray-500 hover:text-gray-700">← Back to orders</Link>
          <h1 className="mt-1 text-2xl font-bold">{order.poNumber}</h1>
          <div className="mt-1 flex items-center gap-3">
            <POStatusBadge status={order.status} />
            <span className="text-xs text-gray-500">Version {order.currentVersion}</span>
          </div>
        </div>
